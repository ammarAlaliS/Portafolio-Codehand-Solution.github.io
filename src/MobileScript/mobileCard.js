...
function MobilecomponentTwo(PopupMobiledata) {
  const { title, closeImg, technologies, img, info, buttonOne, buttonTwo } = PopupMobiledata;
...
function cardComponent(MobileCardData) {
  const { title, info, technologies, img, id, button } = MobileCardData;
...
MobilePopupWindow.addEventListener('click', function(event) {
